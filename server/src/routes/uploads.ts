...
const coverUpload = multer({
  storage: multer.diskStorage({
    ...
    filename: (req, file, cb) => {
      ...
      const ext = path.extname(file.originalname).toLowerCase();
      const safeExt = allowedExtensions.has(ext) ? ext : ".jpg";
      cb(null, `cover-${authedReq.user!.id}-${Date.now()}${safeExt}`);
    },
  }),
...
uploadsRouter.post("/post-cover", requireAuth, (req, res) => {
  coverUpload.single("file")(req, res, (err) => {
    if (err) {
      const message = err instanceof multer.MulterError && err.code === "LIMIT_FILE_SIZE"
        ? "图片太大了，请换一张" : err.message || "封面上传失败";
      return fail(res, 400, 40001, message);
    }
    const file = req.file;
    if (!file) return fail(res, 400, 40001, "请选择一张图片");
    const baseUrl = `${req.protocol}://${req.get("host")}`;
    return ok(res, { url: `${baseUrl}/uploads/covers/${file.filename}` });
  });
});
